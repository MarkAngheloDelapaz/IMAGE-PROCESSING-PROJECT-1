...
static int	InitialBoard( void )
	...
	// I/O board initialize (IN-CY-76)or(IN-CY-119)     2021.06.10<shio>
			if( (Option & NO_INCY76) && (Option & NO_INCY119)){
				sprintf( Comment, "IN-CY-76,119 is not checked" );
				DrawMessage( Comment, DRAW_YELLOW, 1, 0 );
			}
			else if(((Option & NO_INCY119)!=0)&&((Option & NO_INCY76)==0)){
				InitINCY76();
			}
			else if(((Option & NO_INCY119)==0)&&((Option & NO_INCY76)!=0)){
				InitINCY119(); //<z1>
...
void	SetMcacineNo( void )
	{
		...
		R_draw_input_keyboard( DRAW_YELLOW, DISP_H2, MachineName, machine, "Input Mcacine No.", 0 );
...
*/
		...
		//<z1>			if((Option & NO_INCY76)==0){
			if( ((Option & NO_INCY119)==0) || ((Option & NO_INCY76)==0) ){ //<z1> <shio>
				SetsubiDataCheck();
			}
		return ret;
	}
...
int		TP_SearchGuide( void )
	{
		...
		// Data check
			if((0<guide_pos[0])&&(0<guide_pos[1])) {
				IGuidePos[DataX] = min( guide_pos[0] + 147, guide_pos[1]+ 147)-offset_x; //11911 add 147
		...
		// 電極左右位置サーチ
			...
			// view
				...
				//	R_DrawCls();
					...
					R_DrawPrintf(DRAW_WHITE, DISP_NORMAL, posi_res_x, posi_res_y   ,"[Chip position (rough)]");
...
//			InspOcOffsetYsize = 2;
	...
	//				}
					...
					if( i_mode & INSPECT_STEP ) {
						R_move_bin_memory( binPn, BinP0, 0, 0, FxSize-1, FySize-1 );
						sprintf( Comment, "left side clear");
						DrawMessage( Comment, DRAW_GREEN, 2, 2 );
						//  BinClsPn( BinP0, 0, 0, FxSize-1, FySize-1 );
					}
...
//			InspOcOffsetYsizeBlack = 2;
	...
	//				}
					...
					if( i_mode & INSPECT_STEP ) {
						R_move_bin_memory( binPn, BinP0, 0, 0, FxSize-1, FySize-1 );
						sprintf( Comment, "left side clear For BlackInsp");
						DrawMessage( Comment, DRAW_GREEN, 2, 2 );
						//  BinClsPn( BinP0, 0, 0, FxSize-1, FySize-1 );
					}
			   ...
			   // right side
				...
				// left side
					...
					if( i_mode & INSPECT_STEP ) {
						R_move_bin_memory( binPn, BinP0, 0, 0, FxSize-1, FySize-1 );
						sprintf( Comment, "upper side clear For BlackInspInsp");
						DrawMessage( Comment, DRAW_GREEN, 2, 2 );
					}
