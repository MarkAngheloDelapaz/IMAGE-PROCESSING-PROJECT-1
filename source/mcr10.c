...
//extern	int				Mark_div_ws1[5][50];									//標印文字分割Data(Bin正方向)White
extern	int				Mark_div_wr[4][50];									//標印文字分割Data(Bin逆方向)White
extern	int				Mark_div_bs[4][50];									//標印文字分割Data(Bin正方向)Black
//extern	int				Mark_div_bs1[5][50];									//標印文字分割Data(Bin正方向)Black
...
static	int				Bin_Ma_l_ini[5][2]	= {{ 50, 100},{ 50, 100},{ 50, 100},{ 50, 100},{ 50, 100}};	//標印検査時の２値化レベル設定範囲
...
//END 1R00E
		...
		//標印傾き計算 - Mark mark inclination calculation
			deg10 = TWA_katamuki();
			i = (300+deg10)/5;
...
//			}
		...
		//標印傾き確認 - Mark mark inclination check
			i = (300+deg10)/5;
			if((i<0)||(i>120)) return(ERROR);
...
//END 1R00E
			...
			//Yup
				if( Mark_zure[2] == 0 ) {
					...
					//	}
					//	if(n1 == sny) break;			// サーチ開始部分からしきい値以上であればスキップする 20170612tanaka
					//}
					for( n=0 ; n<yn; n++ ) {
						for( n1=0; n1<sny; n1++ ) {
							if( yp[n+n1] < sly ) break;
...
//				}
			...
			if( i_mode & INSPECT_STEP ) {
				...
				//文字検査データ - Character inspection data
				div_n = Mark_div_x * Mark_div_y;
				for( n=0; n<TWA_R_n; n++ ) {
					for( n1=0; n1<div_n; n1++ ) {
						//検査座標設定 - Inspection coordinates setup
						x = x0 + Mark_rx[i][n][n1];
						y = y0 + Mark_ry[i][n][n1];
						xs = x - 5;			//20170517
						xe = x + 4;
...
//					R_get_step_histgram( xs, ys, xe, ye, Gray_address[2] );
					...
					for( n=1; n<=255; n++ ) bright_n += Bright[n];
					bright_n = bright_n/5 + 1;
				//No marking character 有り judgment level 
					//min level
					cl = 0;
					for( n=1; n<255; n++ ) {
						cl += Bright[n];
...
//					if((TWA_Spec==SpecJPW)&&(cl<=4)) return(ERROR);
		...
		//1文字毎にレベル設定（１８０°反転）- It is a level setup (180-degree reversal) for every character.
			for( cn=0; cn<TWA_R_n; cn++ ) {
				//検査範囲設定
...
//END BELOW 1R00E
			...
			//文字計測値初期化 - Character measurement value initialization
			...
			//検査文字指定 - Inspection character specification
				if( TWA_R_n == 3 ) {
