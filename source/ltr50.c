...
static  int				Bin_C2YsizeLevel		= 110;
...
static void	TWA_G2_level( int grayPn, int *level, int *l_lo, int *l_up )
	...
	//	xe = G2_pos[1] - 10;
		ys = G2_pos[2] + 5;
		ye = G2_pos[3] - 5;
		xs	= ElecPosRough[DataUpper][PosXmin]+20;
		xe	= ElecPosRough[DataUpper][PosXmax]-20;
...
}	// LTR_SearchApYpos( int binPn ) end
...
int		TWA_Ap_search_le( int binPn ,int mode)
	{
		int					ye, xs, xl, xe;
		int					G2_ys, G2_ye;
		int					rx, ry, rl;
...
int		TWA_Ap_search_ri( int binPn ,int mode)
	{
		int					ye, xs, xe, xl;
		int					G2_ys, G2_ye;
		int					rx, ry, rl;
...
static int		TWA_kensa_Ap( int binPn ,int mode)
	{
		...
		Ap_l_ex = TWA_Ap_search_le(binPn,mode);							//外形左検査
		Ap_r_ex = TWA_Ap_search_ri(binPn,mode);							//外形右検査
...
int		TWA_Insp_ApLTR( int mode )
	{
		...
		if( (1==bIsLTR50)&&(0!=(Option & BACKINSP_MODE)) ){			//v2.11
			level = g2level-70;
		}else{
			level = g2level-10;
		}
...
int		TWA_G2_ES_searchLTR( int mode )
	{
		int					grayPn	= InspGrayPn;
		int					binPn	= InspBinPn;
		int					i_mode	= R_CS_GetInspectMode();
...
int		TWA_JudgeC2SizeY( int bin_pn )
	{
		int						xs, xe;
		int						ys, ye;
		int						rx, ry, rl, rc;
	...
	}	// TWA_JudgeC2SizeY( int bin_pn ) end


/*********************************************************************************

	TWA_InspC2Ysize( void )

*********************************************************************************/

int		TWA_InspC2Ysize( void )
	{
		int						grayPn	= InspGrayPn;
		int						binPn	= InspBinPn;
		...
		//ノイズ除去処理
			...
			// Judge C2 X size
				if( TWA_JudgeC2SizeY( binPn ) != OK ) return ERROR;

		return OK;

	}	// TWA_InspC2Ysize( void ) end
						...
						if( bad_n >= judge_sd_n ) {
							// disp
								if( i_mode & INSPECT_STEP ) {
									...
									R_DrawPrintf(DRAW_WHITE, DISP_NORMAL, 0, 30   ,"[G2_Judge_l_SDl Le]");
