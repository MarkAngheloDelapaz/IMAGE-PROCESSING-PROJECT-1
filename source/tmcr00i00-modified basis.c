...
int		TP_SearchElect( void )
	{
		...
		// Gray to Bin
            ...
            R_gray2bin( IGuideEsLevel.nData, IGuidePos[1], Index_pos[0], xsize, ysize, Gray_address[grayPn], Bin_address[binPn] ); // 11911 bin image based on set parameter of i guide
			// --- Viwe
			if( i_mode & INSPECT_STEP ) {
				R_move_gray_memory( grayPn, GrayP0, 0, 0, FxSize-1, FySize-1 );
        R_move_bin_memory( binPn, BinP0, 0, 0, FxSize-1, FySize-1 ); //saved bin image
				// --- comment
		...
		// ノイズ除去
            ...
            if( i_mode & INSPECT_STEP ) {
                ...
                // --- comment
                sprintf( Comment, "Expand filter1 " );
                DrawMessage( Comment, DRAW_GREEN, 2, 2 );
            }
            //Expand filter 2
            FilterBinExpand(  &BinFilter, Ap_expand_x,  Ap_expand_y );
            if( i_mode & INSPECT_STEP ) {
                ...
                R_move_bin_memory( binPn, BinP0, 0, 0, FxSize-1, FySize-1 );
                // --- comment
                sprintf( Comment, "Expand filter2 " );
                DrawMessage( Comment, DRAW_GREEN, 2, 2 );
            }
            */
            /*
            ...
            if( i_mode & INSPECT_STEP ) {
                ...
                R_move_bin_memory( binPn, BinP0, 0, 0, FxSize-1, FySize-1 );
                // --- comment
                sprintf( Comment, "Contrct filter1 " );
                DrawMessage( Comment, DRAW_GREEN, 2, 2 );
            }
			FilterBinContrct( &BinFilter, Ap_contrct_x, Ap_contrct_y );
            if( i_mode & INSPECT_STEP ) {
                ...
                R_move_bin_memory( binPn, BinP0, 0, 0, FxSize-1, FySize-1 );
                // --- comment
                sprintf( Comment, "Contrct filter2 " );
                DrawMessage( Comment, DRAW_GREEN, 2, 2 );
            }
            */
           // FilterBinExpand(  &BinFilter, Ap_expand_x,  Ap_expand_y );
...
int		TP_SearchElecYroughLTR( int binPn )
	{
		...
		// Upper
			...
			if( ry >= ymax ) {
                if(  mode == ES_WHITE )	ans_up = OK;//ERROR
                else					ans_up = OK;//ERROR2
			} else {
				ans_up = OK;
				// --- Viwe
				if( i_mode & INSPECT_STEP ) {
...
//					R_DrawCls();
		...
		// Lower
			...
			if( ry <= ymin ) {
                if(  mode == ES_WHITE )	ans_lo = OK; //ERROR
                else					ans_lo = OK; //ERROR2
			} else {
				ans_lo = OK;
				// --- Viwe
				if( i_mode & INSPECT_STEP ) {
			...
			// 白色、面積の大きい上位２つのブローブを電極と判断する。
				...
				// Draw search bin pn
					if( i_mode & INSPECT_STEP ) {
                        ...
                        // --- 11911 center area box chip search
                        area.left = Chip_left;
		...
		// Regression
			...
			// View
				if( i_mode & INSPECT_STEP ) {
					...
					if( (!bIsLTR50) && (!bIsLTR18) ){	// normal
						if((side==PosYmin)||(side==PosYmax)||(side==6)||(side==7)) {
                            xmin = 5;//11911 try to change this val = 5
							xmax = FxSize-5;
